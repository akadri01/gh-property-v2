...
import {RenderFileInput, renderFormInput, renderSelectField, renderTextarea, renderCheckbox} from '../../helpers/reduxForm';
import { postAdvert } from "../../redux/actions";
import {premisesTypeSelectField,featuresCheckboxFieldList,yesNoSelectField,purposeSelectField,balconyQtySelectField,bathroomQtySelectField,floorQtySelectField,postedBySelectField, premisesAgeSelectField, roomsSelectField, locationRegionSelectField, locationTownSelectField} from '../shared/data';
          ...
          <section className="desktop-flex-container">
            <div className="desktop-flex">
              <Field name="premises_type" component={renderSelectField} label="Type of the premises" validate={required()}>
                { premisesTypeSelectField.map(option => <option value={option.value} key={option.value}>{option.text}</option>) }
              </Field>
            </div>
            <div className="desktop-flex">
              <Field name="advert_type" component={renderSelectField} label="Advert type">
                { purposeSelectField.map(option => <option value={option.value} key={option.value}>{option.text}</option>) }
              </Field>
            </div>
            <div className="desktop-flex">
            ...
            <div className="desktop-flex">
              <Field name="rooms_qty" component={renderSelectField} label="Rooms quantity" validate={required()}>
                { roomsSelectField.map(option => <option value={option.value} key={option.value}>{option.text}</option>) }
              </Field>
            </div>
            <div className="desktop-flex">
              <Field name="region" component={renderSelectField} label="Region of the premises" validate={required()}>
                { locationRegionSelectField.map(option => <option value={option.value} key={option.value}>{option.text}</option>) }
              </Field>
            </div>
            <div className="desktop-flex">
              <Field name="town" component={renderSelectField} label="Town of the premises" validate={required()}>
                { locationTownSelectField.map(option => <option value={option.value} key={option.value}>{option.text}</option>) }
              </Field>
            </div>
            <div className="desktop-flex">
            ...
            <div className="desktop-flex">
              <Field name="phone" label=" Phone number" placeholder=" e.g.  0200 290 823" type="number" component={renderFormInput} validate={[required(), length({min: 8, max: 14})]}/>
            </div>
            <div className="desktop-flex">
              <Field name="price" label=" Price (GH₵)" placeholder=" e.g. 85000" type="number" component={renderFormInput} validate={[required(), length({min: 2, max: 11})]}/>
            </div>
            <div className="desktop-flex">
          ...
          <div className="console__post-advert-form-features-container">
            {    
              featuresCheckboxFieldList.map(({labelAndValue, idAndName}) => {
                return <Field key={idAndName} name={idAndName} labelAndValue={labelAndValue} id={idAndName} component={renderCheckbox}/>
              })
            }
          </div>
