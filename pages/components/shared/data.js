export const towns = ['Accra', 'Kumasi', 'Sekondi', 'Sunyani', 'Tamale', 'Takoradi', 'Obuasi', 'Tema', 'Ashaiman'];

export const seoLinks = ['Buy house in Accra', 'Buy shop in Accra', 'Buy office in Accra'];

export const typeLinks = ['house','shop','office','land'];

export const premisesTypeSelectField = [
  {value:'', text:'Select premises type'},
  {value:'house', text:'House'},
  {value:'flat', text:'Flat'},
  {value:'shop', text:'Shop'},
  {value:'land', text:'Land'},
  {value:'building', text:'Building'}
];

export const purposeSelectField = [
  {value:'sale', text:'For sale'},
  {value:'rent', text:'To rent'}
];

export const postedBySelectField = [
  {value:'owner', text:'By owner'},
  {value:'agency', text:'By real estate agent'}
];

export const roomsSelectField = [
  {value:'', text:'Select room quanity'},
  {value:'1+0', text:'Studio 1+0'},
  {value:'1+1', text:'1+1'},
  {value:'2+0', text:'2+0'},
  {value:'2+1', text:'2+1'},
  {value:'2+2', text:'2+2'},
  {value:'3+0', text:'3+0'},
  {value:'3+1', text:'3+1'},
  {value:'3+2', text:'3+2'},
  {value:'4+0', text:'4+0'},
  {value:'4+1', text:'4+1'},
  {value:'4+2', text:'4+2'},
  {value:'4+3', text:'4+3'},
  {value:'4+4', text:'4+4'},
  {value:'5+1', text:'5+1'},
  {value:'5+2', text:'5+2'},
  {value:'5+3', text:'5+3'},
  {value:'5+4', text:'5+4'},
  {value:'6+1', text:'6+1'},
  {value:'6+2', text:'6+2'},
  {value:'6+3', text:'6+3'},
  {value:'7+1', text:'7+1'},
  {value:'7+2', text:'7+2'},
  {value:'7+3', text:'7+3'},
  {value:'8+1', text:'8+1'},
  {value:'8+2', text:'8+2'},
  {value:'8+3', text:'8+3'},
  {value:'8+4', text:'8+4'},
  {value:'9+1', text:'9+1'},
  {value:'9+2', text:'9+2'},
  {value:'9+3', text:'9+3'},
  {value:'9+4', text:'9+4'},
  {value:'10+1', text:'10+1'},
  {value:'10+2', text:'10+2'},
  {value:'10+3', text:'10+3'},
  {value:'10+4', text:'10+4'},
  {value:'10+5', text:'10+5'}
];

export const locationRegionSelectField = [
  {value:'',text:'Select region'},
  {value:"greater_accra", text:'Greater Accra'},
  {value:"central", text:'Central'},
  {value:"eastern", text:'Eastern'},
  {value:"western", text:'Western'},
  {value:"ashanti", text:'Ashanti'},
  {value:"northern", text:'Northern'},
  {value:"upper_east", text:'Upper East'},
  {value:"upper_west", text:'Upper West'},
  {value:"volta", text:'Volta'},
  {value:"brong_ahafo", text:'Brong Ahafo'}
];

export const locationTownSelectField = [
  {value:'',text:'Select town'},
  {value:'accra',text:'Accra'},
  {value:'kumasi',text:'Kumasi'},
  {value:'sekondi',text:'Sekondi'},
  {value:'sunyani',text:'Sunyani'},
  {value:'tamale',text:'Tamale'},
  {value:'takoradi',text:'Takoradi'},
  {value:'obuasi',text:'Obuasi'},
  {value:'tema',text:'Tema'},
  {value:'ashaiman',text:'Ashaiman'},
  {value:'adenta',text:'Adenta'},
  {value:'aflao',text:'Aflao'},
  {value:'agogo',text:'Agogo'},
  {value:'ahwiaa',text:'Ahwiaa'},
  {value:'akatsi',text:'Akatsi'},
  {value:'akwatia',text:'Akwatia'},
  {value:'amanfrom',text:'Amanfrom'},
  {value:'anloga',text:'Anloga'},
  {value:'asamankese',text:'Asamankese'},
  {value:'atebubu',text:'Atebubu'},
  {value:'awoshie',text:'Awoshie'},
  {value:'axim',text:'Axim'},
  {value:'bawku',text:'Bawku'},
  {value:'bekwai',text:'Bekwai'},
  {value:'berekum',text:'Berekum'},
  {value:'bibiani',text:'Bibiani'},
  {value:'bimbilla',text:'Bimbilla'},
  {value:'bolgatanga',text:'Bolgatanga'},
  {value:'buduburam',text:'Buduburam'},
  {value:'dome',text:'Dome'},
  {value:'dzodze',text:'Dzodze'},
  {value:'effiduase',text:'Effiduase'},
  {value:'ejura',text:'Ejura'},
  {value:'elmina',text:'Elmina'},
  {value:'gbawe',text:'Gbawe'},
  {value:'ho',text:'Ho'},
  {value:'hohoe',text:'Hohoe'},
  {value:'kade',text:'Kade'},
  {value:'kintampo',text:'Kintampo'},
  {value:'koforidua',text:'Koforidua'},
  {value:'konongo',text:'Konongo'},
  {value:'lashibi',text:'Lashibi'},
  {value:'madina',text:'Madina'},
  {value:'mampong',text:'Mampong'},
  {value:'mandela',text:'Mandela'},
  {value:'mankessim',text:'Mankessim'},
  {value:'mim',text:'Mim'},
  {value:'east',text:'East'},
  {value:'nkawkaw',text:'Nkawkaw'},
  {value:'nkoranza',text:'Nkoranza'},
  {value:'nsawam',text:'Nsawam'},
  {value:'nsuatre',text:'Nsuatre'},
  {value:'oduponkpehe',text:'Oduponkpehe'},
  {value:'prestea',text:'Prestea'},
  {value:'sakumono',text:'Sakumono'},
  {value:'savelugu',text:'Savelugu'},
  {value:'suhum',text:'Suhum'},
  {value:'taifa',text:'Taifa'},
  {value:'tarkwa',text:'Tarkwa'},
  {value:'techiman',text:'Techiman'},
  {value:'wa',text:'Wa'},
  {value:'wenchi',text:'Wenchi'},
  {value:'winneba',text:'Winneba'},
  {value:'yeji',text:'Yeji'},
  {value:'yendi',text:'Yendi'},
  {value:'agona_swedru',text:'Agona Swedru'},
  {value:'akim_oda',text:'Akim Oda'},
  {value:'assin_fosu',text:'Assin Fosu'},
  {value:'cape_coast',text:'Cape Coast'},
  {value:'dormaa_ahenkro',text:'Dormaa Ahenkro'},
  {value:'new_achimota',text:'New Achimota'},
  {value:'new_tafo',text:'New Tafo'},
  {value:'dunkwa-on-offin',text:'Dunkwa-on-Offin'},
  {value:'tema_new_town',text:'Tema New Town'}
];

export const premisesAgeSelectField = [
  {value:"0", text:'New'},
  {value:"1", text:'1 year'},
  {value:"2", text:'2 years'},
  {value:"3", text:'3 years'},
  {value:"4", text:'4 years'},
  {value:"5", text:'5 years'},
  {value:"9", text:'5 -10 years'},
  {value:"14", text:'10 - 15 years'},
  {value:"19", text:'15 - 20 years'},
  {value:"29", text:'20 - 30 years'},
  {value:"49", text:'30 - 50 years'},
  {value:"99", text:'50 - 100 years'},
  {value:"199", text:'100 - 200 years'},
  {value:"499", text:'200 - 500 years'} 
];

export const floorQtySelectField = [
  {value:'',text:'Select floor'},  
  {value:'1',text:'1'},  
  {value:'2',text:'2'},  
  {value:'3',text:'3'},  
  {value:'4',text:'4'},  
  {value:'5',text:'5'},  
  {value:'6',text:'6'},  
  {value:'7',text:'7'},  
  {value:'8',text:'8'},  
  {value:'9',text:'9'},  
  {value:'10',text:'10'},   
  {value:'11',text:'11'},   
  {value:'12',text:'12'},   
  {value:'13',text:'13'},   
  {value:'14',text:'14'},   
  {value:'15',text:'15'},   
  {value:'16',text:'16'},   
  {value:'17',text:'17'},   
  {value:'18',text:'18'},   
  {value:'19',text:'19'},   
  {value:'20',text:'20'},   
  {value:'21',text:'21'},   
  {value:'22',text:'22'},   
  {value:'23',text:'23'},   
  {value:'24',text:'24'},   
  {value:'25',text:'25'},   
  {value:'26',text:'26'},   
  {value:'27',text:'27'},   
  {value:'28',text:'28'},   
  {value:'29',text:'29'},   
  {value:'30',text:'30'},   
  {value:'31',text:'31'},   
  {value:'32',text:'32'}  
];

export const bathroomQtySelectField = [
  {value:'',text:'Select bathroom quantity'},  
  {value:'1',text:'1'},  
  {value:'2',text:'2'},  
  {value:'3',text:'3'},  
  {value:'4',text:'4'},  
  {value:'5',text:'5'},  
  {value:'6',text:'6'}
];

export const balconyQtySelectField = [
  {value:'',text:'Select balcony quantity'},  
  {value:'0',text:'No balcony'},  
  {value:'1',text:'1'},  
  {value:'2',text:'2'},  
  {value:'3',text:'3'},
  {value:'4',text:'4'}
];
export const yesNoSelectField = [
  {value:'no',text:'No'},
  {value:'yes',text:'Yes'}
];

export const featuresCheckboxFieldList = [
  {idAndName: 'features1', labelAndValue: 'Air conditioning'},
  {idAndName: 'features2', labelAndValue: 'Private parking'},
  {idAndName: 'features3', labelAndValue: 'Private terrace'},
  {idAndName: 'features4', labelAndValue: 'Swimming Pool'},
  {idAndName: 'features5', labelAndValue: 'Built in Barbeque'},
  {idAndName: 'features6', labelAndValue: 'Private Garden'},
  {idAndName: 'features7', labelAndValue: 'Jacuzzi'},
  {idAndName: 'features9', labelAndValue: 'Fire Alarm'},
  {idAndName: 'features10', labelAndValue: 'American Kitchen'},
  {idAndName: 'features11', labelAndValue: 'Bath Tub'},
  {idAndName: 'features12', labelAndValue: 'Built-In Oven'},
  {idAndName: 'features13', labelAndValue: 'Built-in Closet'},
  {idAndName: 'features14', labelAndValue: 'Cellar'},
  {idAndName: 'features15', labelAndValue: 'Double Glazing'},
  {idAndName: 'features16', labelAndValue: 'Elevators'},
  {idAndName: 'features17', labelAndValue: 'Furniture'},
  {idAndName: 'features18', labelAndValue: 'Built-in Kitchen'},
  {idAndName: 'features19', labelAndValue: 'Laminated Flooring'},
  {idAndName: 'features20', labelAndValue: 'PVC Framing'},
  {idAndName: 'features21', labelAndValue: 'Parquet Floor'},
  {idAndName: 'features22', labelAndValue: 'Steel Door'},
  {idAndName: 'features23', labelAndValue: 'Fire Escape'},
  {idAndName: 'features24', labelAndValue: 'Generator'},
  {idAndName: 'features25', labelAndValue: 'Water Tank'}
]
